    ...
    while (std::getline(f,line)) {
        ...
        for (int i = 0; i < line.size(); i++) {
            ...
            if (std::isdigit(ch)) {
                ...
                if (curToken.type == Type::EMPTY) {
                    curToken.type = Type::NUMBER;
                }
                // if its of its own type continue
                else if (curToken.type == Type::NUMBER){}
                // if current token is a letter then keep appending it to the token of type letter
                else if (curToken.type == Type::LETTER) {
                ...
                // new token of type number
                    ...
                    if (curToken.type == Type::LETTER) letterStateCheck(curToken);
            ...
            // be a function
                ...
                if (curToken.type == Type::EMPTY) {
                    curToken.type = Type::LPARANTHESIS;
                    curToken.lexeme += ch;
                }
                else {
                    // if previous token is of type letter and its not a keyword, then that means it has to be a function
                    if (curToken.type == Type::LETTER) {

                        letterStateCheck(curToken);

                        if (curToken.type == Type::IDENTIFIER) curToken.type = Type::FUNCTION;

                    }
                    tokens.push_back(curToken);

                    curToken = {Type::LPARANTHESIS, "("};
                }
            }
            else if (ch == ')') {
                if (curToken.type == Type::EMPTY) {
                    curToken.type = Type::RPARANTHESIS;
                    curToken.lexeme += ch;
                }
                else {
                    // check to see if current token type equal to letter but actually end up being a keyword or identifer
            ...
            // which is allowed to have spaces
                ...
                if (curToken.type != Type::EMPTY) tokens.push_back(curToken);
